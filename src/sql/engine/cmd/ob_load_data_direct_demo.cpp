...
thread_local int thread_idx_frag;
...
int ObLoadSSTableWriter::init_macro_block_writer(const ObTableSchema *table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    for (int i = 0; i < storage::THREAD_NUM; ++i) {
      if (OB_FAIL(data_store_desc_[i].init(*table_schema, ls_id_, tablet_id_, MAJOR_MERGE, 1))) {
        LOG_WARN("fail to init data_store_desc", KR(ret), K(tablet_id_));
        break;
      }
      data_store_desc_[i].sstable_index_builder_ = &sstable_index_builder_;
      ObMacroDataSeq data_seq;
      data_seq.set_parallel_degree(i);
      macro_block_writer_[i] = new ObMacroBlockWriter;
      if (OB_FAIL(macro_block_writer_[i]->open(data_store_desc_[i], data_seq))) {
        LOG_WARN("fail to init macro block writer", KR(ret), K(data_store_desc_[i]), K(data_seq));
        break;
      }
    }
  }
...
void ObLoadSSTableWriter::close_macro() {
  macro_block_writer_[thread_idx_block_writer]->close();
  delete macro_block_writer_[thread_idx_block_writer];
}
...
int ObLoadDataDirectDemo::inner_init(ObLoadDataStmt &load_stmt)
{
  int ret = OB_SUCCESS;
  thread_idx_ = 0;
  load_args_ = &load_stmt.get_load_arguments();
  ...
  for (int i = 0; i < storage::THREAD_NUM; ++i) {
    ...
    // init row_caster_
    if (OB_FAIL(row_caster_[i].init(table_schema_, *field_or_var_list_))) {
      LOG_WARN("fail to init row caster", KR(ret));
      return ret;
    }
  }
...
int ObLoadDataDirectDemo::do_load()
{
  ...
  auto read_and_append1 = [&thread_idx_global, &ret_global, &ob_mutex1, /*&ob_mutex2,*/ this](){
    ...
    while (true) {
      ...
      if (OB_FAIL(this->buffer_[thread_idx].squash())) {
        LOG_WARN("fail to squash buffer", KR(ret));
        ret_global = ret;
        ob_mutex1.unlock();
      } else if (OB_FAIL(this->file_reader_.read_next_buffer(this->buffer_[thread_idx]))) {
